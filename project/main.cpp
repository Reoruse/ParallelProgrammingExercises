...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    cout << "       >>> WAIT, WHERE'S THE RAINBOW FILTER? <<<" << endl;
}
...
public:
    void log(string msg) {
        historial.push_back((SECRET ? ">> " : "> ") + msg);
    ...
    void dibujar(CImg<unsigned char>& canvas, int x, int y, int w, int h) {
        ...
        unsigned char bgSec[] = {10, 30, 30};
        unsigned char bordeSec[] = {57, 197, 187};
        unsigned char textoSec[] = {255, 255, 255}; 

        unsigned char* cBg = SECRET ? bgSec : bg;
        unsigned char* cBorde = SECRET ? bordeSec : white;
        unsigned char* cTexto = SECRET ? textoSec : texto;

        canvas.draw_rectangle(x, y, x + w, y + h, cBg);
        canvas.draw_rectangle(x, y, x + w, y + h, cBorde, 1.0f, ~0U);
        canvas.draw_text(x + 5, y - 12, SECRET ? "VIRTUAL DIVA TERMINAL" : "LOG / STATUS", cBorde, 0, 1, 11);
...
void aplicarFiltro01(CImg<unsigned char>& img) {
    #pragma omp parallel for if(usarMultithread)
    cimg_forXY(img, x, y) {
        ...
        img(x, y, 0) = (unsigned char)(r * 0.2);
        img(x, y, 1) = (unsigned char)std::min(255.0, g * 1.2 + 20);
        img(x, y, 2) = (unsigned char)std::min(255.0, b * 1.4 + 40);
    }
}
...
int main() {
    ...
    CImg<unsigned char> imgBase(MAX_IMG_W, MAX_IMG_H, 1, 3);
    imgBase.draw_rectangle(0, 0, MAX_IMG_W, MAX_IMG_H, colorVacio);
    imgBase.draw_text(MAX_IMG_W/2 - 50, MAX_IMG_H/2, "NO IMAGE", colorTexto, 0, 1, 24);
    ...
    unsigned char SecTeal[] = {57, 197, 187};
    unsigned char SecDark[] = {5, 20, 20};
    unsigned char SecPink[] = {228, 0, 127};
    unsigned char SecCyan[] = {0, 255, 255};

    int contadorClicksSecretos = 0;

    while (!disp.is_closed()) {
        unsigned char* cFondo    = SECRET ? SecDark : (unsigned char*)"\x2b\x2b\x2b";
        unsigned char* cPrimario = SECRET ? SecTeal : orange;
        unsigned char* cReset    = SECRET ? SecPink : red;
        unsigned char* cFiltro   = SECRET ? SecTeal : persian;
        unsigned char* cTextoBtn = SECRET ? white    : black;
        unsigned char* cTitulo   = SECRET ? SecCyan : white;

        CImg<unsigned char> canvas(disp.width(), disp.height(), 1, 3);
        canvas.draw_rectangle(0, 0, disp.width(), disp.height(), cFondo);

        int imgY = 40;
        dibujarTextoNegrita(canvas, MENU_W + 10, 10, "ORIGINAL", cTitulo, 18);
        canvas.draw_image(MENU_W + 10, imgY, imgDisplayOrig);
        
        dibujarTextoNegrita(canvas, MENU_W + MAX_IMG_W + 30, 10, "RESULT", cTitulo, 18);
        canvas.draw_image(MENU_W + MAX_IMG_W + 30, imgY, imgDisplayProc);

        int bx = 10, bw = 210, bh = 35, y = 20;

        // CARGAR
        canvas.draw_rectangle(bx, y, bx+bw, y+bh, cPrimario);
        dibujarTextoNegrita(canvas, bx+60, y+10, "UPLOAD IMAGE", cTextoBtn);
        ...
        // RESET
        canvas.draw_rectangle(bx, y, bx+bw, y+bh, cReset);
        dibujarTextoNegrita(canvas, bx+60, y+10, "FILTERS RESET", cTextoBtn);
        if (disp.button()&1 && disp.mouse_x()>=bx && disp.mouse_x()<=bx+bw && disp.mouse_y()>=y && disp.mouse_y()<=y+bh) {
            datosProcesados = datosOriginales;
            imgDisplayProc = imgDisplayOrig;
        ...
        if(SECRET) colM = usarMultithread ? SecCyan : grey;

        canvas.draw_text(bx + 5, y - 12, "PROCESSING OPTIONS", white, 0, 1, 11);
        canvas.draw_rectangle(bx, y, bx+bw, y+bh, colM);
        dibujarTextoNegrita(canvas, bx+10, y+10, usarMultithread ? "MULTITHREAD" : "SEQUENTIAL", cTextoBtn);
        if (disp.button()&1 && disp.mouse_x()>=bx && disp.mouse_x()<=bx+bw && disp.mouse_y()>=y && disp.mouse_y()<=y+bh) {
            usarMultithread = !usarMultithread;
            terminal.log(usarMultithread ? "Parallel ON" : "Sequential ON");
            disp.wait(200);
        }
        y += 40;

        unsigned char* colG = usarGPU ? turquoise : cornflower;
        if(SECRET) colG = usarGPU ? SecCyan : grey;

        canvas.draw_rectangle(bx, y, bx+bw, y+bh, colG);
        dibujarTextoNegrita(canvas, bx+10, y+10, usarGPU ? "GPU (CUDA)" : "CPU", cTextoBtn);
        if (disp.button()&1 && disp.mouse_x()>=bx && disp.mouse_x()<=bx+bw && disp.mouse_y()>=y && disp.mouse_y()<=y+bh) {
            usarGPU = !usarGPU;
            if(usarGPU) terminal.log("GPU Selected");
            else terminal.log("CPU Selected");
            disp.wait(200);
        }
        y += 65;
        ...
        auto btn = [&](const char* txt, auto func) {
            canvas.draw_rectangle(bx, y, bx+bw, y+bh, cFiltro);
            dibujarTextoNegrita(canvas, bx+70, y+10, txt, cTextoBtn);
            if (disp.button()&1 && disp.mouse_x()>=bx && disp.mouse_x()<=bx+bw && disp.mouse_y()>=y && disp.mouse_y()<=y+bh) {
        ...
        if (!logoImg.is_empty()) {
            ...
            if (disp.button()&1 && disp.mouse_x()>=logoX && disp.mouse_x()<=logoX+logoImg.width() && disp.mouse_y()>=logoY && disp.mouse_y()<=logoY+logoImg.height()) {
                disp.button();
                ...
                if (contadorClicksSecretos == 3) {
                    ...
                    if (SECRET) {
                        terminal.limpiar();
                        invocarSec();
                        terminal.log("EasterEgg by: Cristopher Ochoa");
                        terminal.log("Check the console!");
                    } else {
                        system("cls");
                        terminal.limpiar();
                        terminal.log("Created by: Cristopher Ochoa");
                        terminal.log("System restored.");
                    }
